gameStates.game = {
  ...
  preload: function() {
    ...
    for (var i = 0; i < trees_count; i++) {
      hill = this.trees_front.create(
        (i / trees_count) * game.camera.width + Math.random() * 100,
        200 + Math.random() * 20,
        'tree' + Math.round(Math.random())
      );
    ...
    for (var i = 0; i < trees_count; i++) {
      ...
      hill = this.trees_back.create(
        (i / trees_count) * game.camera.width + Math.random() * 20,
        105 + (1 - treeDist) * 10,
        'tree' + Math.round(Math.random())
      );
      hill.anchor.setTo(0.5, 1);
    ...
    this.particleEmitter = game.add.emitter(
      game.camera.width / 2,
    ...
    this.starEmitter = game.add.emitter(
      game.camera.width / 2,
      game.camera.height / 2,
      200
    );
    ...
    this.counterText = game.add.text(
      game.camera.width / 2,
      this.counterTextY,
      this.myPoints,
      this.bigTextStyle
    ...
    this.clicker.events.onInputDown.add(function() {
      this.handlePedalDownInput();
    }, this);

    this.clicker.events.onInputUp.add(function() {
			this.handlePedalUpInput();
    }, this);
	...
	handlePedalDownInput() {
		if (this.motorCooling || this.pendingBoost) return;
		this.pedalDown = true;
  ...
  onCrossFinishline: function() {
    ...
    game.add
      .tween(this.myCar)
      .to({ x: startX + 100 }, 1000, Phaser.Easing.Quadratic.InOut, true);
    comms.requestVictory();
  },

  hideBoosts: function() {
    if (this._tweenBoostsDisplay && this._tweenBoostsDisplay.stop)
      this._tweenBoostsDisplay.stop();
    this._tweenBoostsDisplay = game.add
      .tween(this.boosts)
      .to(
        { x: game.camera.width },
  ...
  hideClicker: function() {
    if (this._tweenClickerDisplay && this._tweenClickerDisplay.stop)
      this._tweenClickerDisplay.stop();
    this._tweenClickerDisplay = game.add
      .tween(this.clicker)
      .to(
        { y: this.clickerHideY },
        this.hideAnimationTime,
  ...
  onBoostCooldownStart: function(boost) {
    ...
    boost.pendingSprite = game.add.sprite(
      game.camera.width / 2,
      game.camera.height / 2,
      boost.name
    );
    boost.pendingSprite.anchor.setTo(0.5, 0.5);
    this._pendingCooldownBar = new Phaser.Rectangle(
      0,
      -50,
      this.cooldownBarWidth,
      this.cooldownBarHeight
    );
  },
  ...
  update: function() {
    ...
    this.meter.x =
      game.camera.width / 2 + Math.sin(now / 10) * this.meterShakeAmount;
    this.meterShakeAmount = Math.max(0, this.meterShakeAmount - 0.1);
		...
		[1, 2, 3].forEach(number => {
			...
			const boost = this.boosts.children[number - 1];

			if(keys.down[String(number)] && boost) {
				this.onBoostClick(boost);
			}
		});
    ...
    this.meter_pointer.rotation =
      Math.PI * (5 / 6) + (this.rpm / this.maxRpm) * (8 / 6) * Math.PI;
    ...
    this.finishlineSprite.x =
      this.myCar.worldPosition.x +
      this.trackWidth -
      (this.myCar.playerData.points / this.targetPoints) * this.trackWidth;
    this.startLine.x =
      this.myCar.worldPosition.x -
      (this.myCar.playerData.points / this.targetPoints) * this.trackWidth;
    ...
    this.progressBar.x =
      (this.myCar.playerData.points / this.targetPoints) * game.camera.width -
      game.camera.width;
    ...
    this.playersContainer.forEach(function(p) {
      ...
      if (p.playerData.id === comms.id) {
        p.x =
          ((p.playerData.points - this.myCar.playerData.points) /
            this.targetPoints) *
          this.trackWidth;
      } else {
        if (!p.playerData.interpolatedPoints)
          p.playerData.interpolatedPoints = p.playerData.pointsThen;

        syncer = (p.playerData.points - p.playerData.interpolatedPoints) * 0.02;
        //p.playerData.points-p.playerData.pointsThen

        p.playerData.interpolatedPoints =
          p.playerData.interpolatedPoints +
          (p.playerData.points - p.playerData.pointsThen) *
            ((1000 * game.time.physicsElapsed) / this.serverDt);
        p.playerData.interpolatedPoints += syncer;

        p.x =
          ((p.playerData.interpolatedPoints - this.myCar.playerData.points) /
            this.targetPoints) *
          this.trackWidth;
      }
  ...
  render: function() {
    ...
    this.boosts.forEach(function(b) {
      b.cooldownBar.width = Math.max(
        0,
        this.cooldownBarWidth *
          (1 - (now - b.spawnThen) / this.boostMaxCooldown)
      );
    ...
    if (this._pendingCooldownBar && this.pendingBoost) {
      var w = Math.max(
        0,
        this.cooldownBarWidth *
          (1 - (now - this.pendingBoost.spawnThen) / this.boostMaxCooldown)
      );
      this._pendingCooldownBar.x =
        this.pendingBoost.pendingSprite.worldPosition.x -
        this.cooldownBarWidth / 2;
      this._pendingCooldownBar.y =
        this.pendingBoost.pendingSprite.worldPosition.y - 50;
  ...
  positionBoosts: function() {
    ...
    this.boosts.forEach(function(b) {
      ...
      tween = game.add
        .tween(b)
        .to({ x: x, y: 0 }, time, Phaser.Easing.Quadratic.InOut, true);
      index++;
    }, this);
  },
  ...
  addBoost: function(boost) {
    ...
    b.cooldownBar = new Phaser.Rectangle(
      0,
      -50,
      this.cooldownBarWidth,
      this.cooldownBarHeight
  ...
  onBoostClick: function(sprite) {
    if (!this.waitingBoostResponse && !this.pendingBoost) {
			const boost = sprite.model;
      ...
      comms.requestBoost(
        ...
        function(isSuccess) {
          if (isSuccess) {
            ...
            if (Date.now() - sprite.spawnThen < this.boostMaxCooldown) {
              //cooldown remaining
  ...
  applyBoost: function(boost) {
    ...
    var tween = game.add
      .tween(icon.scale)
      .to({ x: 1.3, y: 1.3 }, 300, Phaser.Easing.Quadratic.Out, true);
    tween.onComplete.add(function() {
      this.starEmitter.start(true, 3000, null, 10);
      var shrink = game.add
        .tween(icon.scale)
        .to({ x: 0, y: 0 }, 300, Phaser.Easing.Quadratic.In, true);
